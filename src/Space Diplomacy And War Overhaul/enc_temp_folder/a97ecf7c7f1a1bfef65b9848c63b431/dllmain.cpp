...
member_detour(Load__detour, UTFWin::UILayout, bool(const ResourceKey& resourceKey, bool , uint32_t)) {
	bool detoured(ResourceKey key, bool a, uint32_t b) {
		bool ret = original_function(this, key, a, b);
		if (key.instanceID == 0x6cc06014) {
			int a = 3;
		}
		return ret;
	}
...
void AttachDetours()
{
	...
	Load__detour::attach(GetAddress(UTFWin::UILayout, Load));
}
